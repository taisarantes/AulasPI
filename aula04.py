...
def show_image():
    ...
    try:
        ...
        window['-IMAGE-'].draw_image(data=img_bytes.getvalue(), location=(0, 600))
    except Exception as e:
...
def crop_selected_area(start_x, end_x, start_y, end_y):
    global image_atual
    global previous_state
    ...
    try:
        if image_atual:
            previous_state = image_atual.copy()
            area = (start_x , end_x, start_y, end_y)
            image_atual = image_atual.crop(area)
            show_image(image_atual)
        else:
            sg.popup("Nenhuma imagem aberta.")
    except Exception as e:
        sg.popup(f"Erro ao cortar a imagem: {str(e)}")
...
layout = [
    ...
    [sg.Graph(key='-IMAGE-', canvas_size=(800, 600), change_submits=True, drag_submits=True,graph_bottom_left=(0, 0), graph_top_right=(800, 600))],
]
    ...
    if event in (sg.WINDOW_CLOSED, 'Fechar'):
        ...
        if '-IMAGE-' in values and values['-IMAGE-'] is not None:
            if event.endswith('-IMAGE-+UP'):
                ...
                # end_y = 400 - end_y
                # sg.popup(f"Coordenadas: p1({start_x}, {start_y}), p2({end_x}, {start_y}), p3({start_x}, {end_y}), p4({end_x}, {end_y})")
                crop_selected_area(start_x, end_x, start_y, end_y)
            elif not selecting:
                selecting = True
